...
Bool UA_Question_IsSimpIdentDet1(Obj *head, Obj *con0, Obj *con1, Obj *con2,
                                 /* RESULTS */ Obj **object, Obj **restrict)
{
...
Bool UA_Question_IsLocIdentDet(Obj *con, /* RESULTS */ Obj **object,
                                              Obj **restrict)
{
  return(UA_Question_IsSimpIdentDet(N("location-of"), con, object, restrict));
}
...
{
  Obj	*object, *restrict;
  if (UA_Question_IsLocIdentDet(question, &object, &restrict)) {
    return(UA_QuestionLocation(question, object, restrict, tsr, an, dc));
  } else {
                        ...
                        Discourse *dc)
{
  Obj		*object, *restrict;
  ObjList	*answer;
  if (UA_Question_IsSimpIdentDet(N("near"), question, &object, &restrict)) {
  /* todo: Should obey other restrictions besides class in the intension. */
    if ((answer = SpaceFindNear(NULL, tsr, object, restrict, NULL))) {
