...
void StatClassify(FILE *stream)
{
  ...
  long numAtomicConcepts, numISA, numPartWhole, numSelect, numOtherAssertion;
  long numPhysObjProp, numInference, numLex, numLeo, numArg, numAtGrid;
  long numSubstance, numAttrRel, numAntonym, numRole, numFact, numProper;
  ...
  fprintf(stream, "ASSERTION:ATTRIBUTE %ld\n", numAttrRel);
  ...
  fprintf(stream, "ASSERTION:ROLE %ld\n", numRole);
  fprintf(stream, "ASSERTION:SELPREF %ld\n", numSelect);
