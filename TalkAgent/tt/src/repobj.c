...
{
  if (!ObjTypeOK(obj->type)) return(0);
  if (obj->type == OBJTYPELIST) {
    if (obj->u1.lst.asserted != 0 && obj->u1.lst.asserted != 1) return(0);
    if (obj->u1.lst.len < 0) return(0);
  } else {
...
/* STRING:email-address:"erik@signiform.com" */
