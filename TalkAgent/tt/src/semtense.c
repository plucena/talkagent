...
Obj *TenseFindInflTense(int lang, Obj *tense, Obj *mood)
{
  ...
  if (lang == F_ENGLISH) {
    ...
    for (p = objs; p; p = p->next) {
      if (YES(RE(&TsNA, L(N("eng-infl-mood-of"), I(p->obj, 1), mood)))) {
        r = I(p->obj, 1);
        ObjListFree(objs);
        return(r);
      }
    ...
    for (p = objs; p; p = p->next) {
      if (YES(RE(&TsNA, L(N("fr-infl-mood-of"), I(p->obj, 1), mood)))) {
        r = I(p->obj, 1);
        ObjListFree(objs);
        return(r);
      }
                      ...
                      Obj *maintense)
{
  ...
  if (lang == F_ENGLISH) {
    ...
    for (p = objs1; p; p = p->next) {
      if (YES(RE(&TsNA, L(N("eng-main-tense-of"), I(p->obj, 1), maintense)))) {
        objs2 = RE(&TsNA, L(N("eng-aux-verb-of"), I(p->obj, 1), ObjWild));
    ...
    for (p = objs1; p; p = p->next) {
      if (YES(RE(&TsNA, L(N("fr-main-tense-of"), I(p->obj, 1), maintense)))) {
        objs2 = RE(&TsNA, L(N("fr-aux-verb-of"), I(p->obj, 1), ObjWild));
        for (q = objs2; q; q = q->next) {
          if (ObjIsForLexEntry(I(q->obj, 2), auxverb)) {
            r = I(p->obj, 1);
            ObjListFree(objs1);
