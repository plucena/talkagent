...
Bool PNodeSimilarIn(PNode *pn, ObjList *objs)
{
  ...
  for (p = objs; p; p = p->next) {
    if (PNodeSimilar(pn, p->u.pn)) return(1);
  }
  return(0);
}
...
ObjList *PNodeAppendNonsimilar(ObjList *objs1, ObjList *objs2)
{
  ...
  for (p = objs2; p; p = p->next) {
    if (!PNodeSimilarIn(p->u.pn, objs1)) {
                     ...
                     Discourse *dc)
{
  ...
  switch (pn->type) {
    case PNTYPE_LEXITEM:
      if (pn->lexitem && pn->lexitem->word && pn->lexitem->word[0]) {
        if (DC(dc).lang != FeatureGet(pn->lexitem->features, FT_LANG)) {
        ...
        if (capitalize) buf[0] = CharToUpper((uc)(buf[0]));
