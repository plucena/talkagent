...
class TTKB:
  def __init__(self, ttroot, options):
    ...
    try:
      ...
      printLine(ttroot+'/ttkb/*.txt not found',options)
      printLine('You may need to set TTROOT',options)
      printLine('You may need to run $TTROOT/runtime/grind.sh',options)
      self.OK = 0
    self.clearCache()
  def clearCache(self):
    self.leCache = {}
  ...
  def grep(self,ptn,options):
    printLine('ThoughtTreasure words:',options)
    newline(options)
    self.le.seek(0)
    done=[]
    while 1:
      ...
      if -1 != string.find(s,ptn):
        ...
        if word not in done:
          ...
          printLine(wordString(word,options),options)
    newline(options)
    printLine('ThoughtTreasure objects:',options)
    newline(options)
    self.obj.seek(0)
    done=[]
    while 1:
      ...
      if -1 != string.find(s,ptn):
        ...
        if objname not in done:
          ...
          printLine(objnameString(objname,options),options)
  def __repr__(self):
...
alloptions=['-anc','-assert','-child','-desc','-frame','-parent',
            '-obj','-word','-le','-grep','-html']+qaoptions
...
def cmdObj(h,objname,options):
  obj=h.objGet(objname)
  if not obj:
    printLine('Object '+objname+' not found.',options)
    return
  printLine('ThoughtTreasure object: '+objnameStringNoLink(objname,options),
            options)
  newline(options)
  s=obj.words(h,options)
  if s: printLine(s, options)
  ...
  if involving!=None:
    for a in obj.assertions:
      if deepIn(involving,a) and isa(h,a[0],predClass):
        printLine(assertionFmt1(a,obj.objname,options),options)
  elif predClass!=None:
    for a in obj.assertions:
...
def cmdLeo1(h,N,leo,obj,options):
  ...
  if '-frame' in options and leo.thetas:
    printLine(indentString(10,options)+'*> '+frameFmt(leo.thetas),options)
  if '-assert' in options:
    cmdAssertions(h,obj,None,None,options)
  visited={}
  if '-parent' in options or '-anc' in options:
...
def cmdChildren(h,indent,objname,options,visited):
  ...
  for c in children:
    ...
    printLine(indentString(3*indent,options)+'=> '+
...
def cmdParents(h,indent,objname,options,visited):
  ...
  for c in parents:
    ...
    printLine(indentString(3*indent,options)+'<= '+parent.fmt(h,options),
              options)
    if '-assert' in options:
...
def cmdLe(h,leuid,options):
  le=h.leGet(leuid, options)
  if not le:
    printLine('Lexical entry '+leuid+' not found.',options)
    return
  leword=leUidToWord(le.uid)
  printLine('ThoughtTreasure lexical entry: '+leuid,options)
  newline(options)
...
def cmdWord(h,word,options):
  ...
  if infls==None or len(infls)==0:
    printLine('Word '+word+' not found.',options)
    return
  printLine('ThoughtTreasure word: '+word,options)
  newline(options)
  done=[]
  for infl in infls:
  ...
  for x in argv:
    if len(x) > 0:
      if x[0] == '-' and x not in alloptions:
        printLine('Unknown option'+x,argv)
  return None

def usage(options):
  newline(options)
  printLine('Usage: ttkb key [-keytype] [-searchtype...] [-html]',options)
  printLine('     -html    Dump in HTML format',options)
  printLine('keytype can be:',options)
  printLine('     -grep    Pattern ',options)
  printLine('     -le      Lexical entry uid',options)
  printLine('     -obj     Object name',options)
  printLine('     -word    Word (default)',options)
  printLine('searchtype can be:',options)
  printLine('     -anc     Show ancestor concepts',options)
  printLine('     -assert  Show assertions',options)
  printLine('     -child   Show child concepts',options)
  printLine('     -desc    Show descendant concepts',options)
  printLine('     -frame   Show argument structure frames',options)
  printLine('     -parent  Show parent concepts',options)
  newline(options)
  printLine('Examples:',options)
  printLine("ttkb 'carry-on bag' # Show meanings",options)
  printLine('ttkb put -frame # Show argument structure frames',options)
  printLine('ttkb food -desc # Show hierarchy of foods',options)
  printLine('ttkb tennis-ball -obj -assert',options)
  printLine('  # Show all assertions regarding an object',options)
  printLine('ttkb tennis-ball -obj -anc -assert',options)
  printLine('  # Show all assertions regarding an object and its ancestors',
            options)
  newline(options)
...
def cmdQARole(h,leo,objname,word,roleClass,predClasses,options):
  ...
  for e in scripts:
    ...
    eobj=h.objGet(e[1])
    newline(options)
    printLine('In script '+eobj.fmt(h,options),options)
  ...
  for leo in leos:
    if typ=='role':
      ...
      if cmdQAAction(h,leo,leo.objname,word,predClasses,visited,options):
        found=1
  if not found:
    printLine('No relevant information about '+word+'.',options)
...
def cmdTextToScript(h,word,options):
  ...
  keys=dGlob.keys()
  if len(keys)==0:
    printLine('No script recognized.',options)
    return
...
def cmd(argv):
  ...
  if not os.environ.has_key(var):
    printLine('You need to set the environment variable '+var+':',argv)
    printLine('   export '+var+'=(the root directory of ThoughtTreasure)',argv)
    printLine('You might also want to add this command to your PATH:',argv)
    printLine('   export PATH=$PATH:$TTROOT/python',argv)
    newline(argv)
    ttroot='/usr/local/tt'
  else:
